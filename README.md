...
- **搜索**：发现页右上角搜索按钮进入搜索页；关键词 + 型号 + 类型三重筛选（本地聚合检索：覆盖多个型号分类 × 最新/推荐双排序 × 多页并发抓取后本地匹配，命中率高）
...
- **自动检查更新**：每次启动自动检查 GitHub Releases（我的 → 更新源仓库配置）；有新版本弹窗提示，支持「前往 Releases」或「应用内下载 APK」两种方式更新
- **详情页**：预览图轮播（点击放大、双指缩放）、评分（★ 与评分人数）、下载量、作者、更新时间、资源描述
...
> v1.1.1 搜索移入发现页右上角；v1.1.2 搜索范围扩大（多分类×多排序并发）；v1.1.3 图片点击放大；
> v1.1.4 自动检查更新（GitHub Releases）+ 关于作者信息。
...
每次发布新版本必须**同步修改三处**，否则自动更新或覆盖安装会出问题：

1. **`pubspec.yaml`**：`version: 1.1.5+15`（versionName 更新 + **versionCode 递增**——Android 按 versionCode 判断新旧，不递增会导致用户无法覆盖安装，只能卸载重装）
2. **`lib/core/config.dart`**：`AppConfig.appVersion = '1.1.5'`（自动更新检查比较用，须与 versionName 一致）
3. **GitHub Releases**：发布 tag 用 `v1.1.5`，正文写更新说明，附件上传 APK
